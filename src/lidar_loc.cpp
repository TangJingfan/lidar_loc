...
void initialPoseCallback(
  ...
  try {
    ...
    geometry_msgs::TransformStamped transformStamped = tfBuffer.lookupTransform(
        base_frame, laser_frame, ros::Time(0), ros::Duration(1.0));
    ...
    tf2::Quaternion q(
        laser_pose.pose.orientation.x, laser_pose.pose.orientation.y,
        laser_pose.pose.orientation.z, laser_pose.pose.orientation.w);
    ...
    // 6. 将地图坐标转换为裁切后的地图坐标
    lidar_x = (x - map_msg.info.origin.position.x) / map_msg.info.resolution -
              map_roi_info.x_offset;
    lidar_y = (y - map_msg.info.origin.position.y) / map_msg.info.resolution -
              map_roi_info.y_offset;

    // 7. 设置yaw角度
    lidar_yaw = -yaw;
    clear_countdown = 30;
  } catch (tf2::TransformException &ex) {
    ROS_WARN("无法获取从 %s 到 %s 的转换: %s", base_frame.c_str(),
             laser_frame.c_str(), ex.what());
  }
}
...
void crop_map() {
  ...
  // 把地图数据转换成图片
  cv::Mat map_raw(info.height, info.width, CV_8UC1,
                  cv::Scalar(128)); // 灰色背景
...
void scanCallback(const sensor_msgs::LaserScan::ConstPtr &msg) {
  ...
  while (ros::ok()) {
    if (!map_cropped.empty()) {
      // 计算三种情况下的雷达点坐标数组
      std::vector<cv::Point2f> transform_points, clockwise_points,
          counter_points;
      ...
      for (const auto &point : scan_points) {
        ...
        transform_points.push_back(
            cv::Point2f(rotated_x + lidar_x, lidar_y - rotated_y));
...
void pose_tf() {
  ...
  try {
    odom_to_base =
        tfBuffer.lookupTransform(odom_frame, laser_frame, ros::Time(0));
  } catch (tf2::TransformException &ex) {
    ROS_WARN("%s", ex.what());
    return;
  }
  ...
  // 计算 yaw 角度
  tf2::Quaternion q_tf2(map_to_odom_msg.transform.rotation.x,
...
int main(int argc, char **argv) {
  ...
  ros::Subscriber initial_pose_sub =
      nh.subscribe("initialpose", 1, initialPoseCallback);
